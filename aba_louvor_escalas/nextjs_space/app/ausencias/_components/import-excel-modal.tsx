...
export function ImportExcelModal({ onClose }: ImportExcelModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/ausencias/import', {
        method: 'POST',
        body: formData
      });
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
        ...
        >
          <div className="flex items-center justify-between p-6 border-b border-gray-800">
            <h2 className="text-2xl font-bold">Importar Ausências (Excel)</h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
              ...
              <ul className="text-sm text-gray-400 space-y-1">
                <li>• <strong>email</strong>: Email do voluntário</li>
                <li>• <strong>dataInicio</strong>: Data de início (DD/MM/AAAA)</li>
                <li>• <strong>dataFim</strong>: Data de fim (DD/MM/AAAA) - opcional</li>
                <li>• <strong>motivo</strong>: Motivo da ausência (opcional)</li>
              </ul>
            </div>
            ...
            {result && (
              <div className={`p-4 rounded-lg ${
                ...
                <p className={result.errors?.length === 0 ? 'text-green-400' : 'text-yellow-400'}>
                  {result.success} ausência(s) importada(s) com sucesso!
                </p>
